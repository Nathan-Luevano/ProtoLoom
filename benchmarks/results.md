...
A fresh cProfile run after all three fixes above showed the remaining ~52s
on Signal dominated by genuine per-dex CPU work, not redundant parsing:
`container/dex.py`'s `iter_code_items` (21.1s), `extract/lite.py`'s
`extract_lite` (18.9s), and `decode/lite.py`'s `decode_lite_finding` (13.7s)
— pure Python instruction-walking over each of Signal's 8 `classes*.dex`
files, one dex at a time. This is CPU-bound and holds the GIL, so (unlike
the `protoc` waits above) it can't be sped up with threads; it needs real
process-level parallelism to overlap across cores. `_find_lite` now runs
each input dex's `extract_lite`/`decode_lite_finding` pass in its own worker
process (`ProcessPoolExecutor.map`, so combining results afterward
preserves the exact input order — unchanged output naming and first-failure
error attribution). Each worker parses its own `DexFile` from the raw dex
bytes rather than sharing the thread-side `_cached_dex` dict, which can't
cross a process boundary; `DexFile` construction is fast next to the
instruction-walking work it feeds, so the extra parse is cheap. The pool is
skipped entirely for a single dex input, where its startup cost isn't worth
paying. `_find_wire` and `_find_grpc` are unchanged for now — `_find_lite`
alone accounted for the large majority of the remaining CPU time.

The default `fork` start method segfaulted intermittently on real APKs
(reproduced on Molly, `dmesg` showing a real segfault in the `protoloom`
process, `BrokenProcessPool` raised afterward) — this project's protobuf
backend is `upb`, a C extension, and forking a process that has already
touched its global state is not guaranteed safe. Switching the pool to
`spawn` (`multiprocessing.get_context("spawn")`, which re-execs a clean
interpreter per worker instead of duplicating parent memory) reproduced
zero segfaults across a stress run of 9 back-to-back real extractions
(Mullvad, Molly, Smartspacer x3 each) after the switch, versus a real
failure on the very first repeat of the same sequence under `fork`.

| APK | Wall time (prior fixes only) | Wall time (+ process pool) |
|---|---:|---:|
| Signal 8.22.2 | 26.1s | 19.3s |

Signal's output stayed byte-for-byte identical (`diff -rq`, zero
differences across all 789 files, two repeated runs); Mullvad (130 files),
Molly (589 files), and Smartspacer (70 files) reproduced their documented
counts and zero bail-outs unchanged, each verified with `diff -rq` against
their own prior output.
