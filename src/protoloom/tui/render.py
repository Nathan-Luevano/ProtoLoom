...
def _render(item: RenderableType, width: int) -> str:
    ...
    console = Console(
        file=stream,
        width=max(20, min(width, MAX_RENDER_WIDTH)),
        force_terminal=True,
...
def _text(value: object) -> str:
    rendered = str(value)
    if len(rendered) <= MAX_RENDER_TEXT_LENGTH:
        return rendered
    return f"{rendered[: MAX_RENDER_TEXT_LENGTH - 3]}..."
...
def _add_messages(
    ...
    for message in _objects(value):
        if not budget.consume(parent):
            return
        name = _text(message.get("name", "unnamed"))
        confidence = _text(message.get("confidence", "unknown"))
        branch = parent.add(
        ...
        for field in _objects(message.get("fields")):
            if not budget.consume(branch):
                return
            field_confidence = _text(field.get("confidence", "unknown"))
            branch.add(
                escape(
                    f"{_text(field.get('number', '?'))}  "
                    f"{_text(field.get('name', 'unnamed'))}: "
                    f"{_text(field.get('type_name', 'unknown'))} [{field_confidence}]"
                )
            )
...
def render_schema(
    ...
    root = Tree(f"[bold]{escape(_text(schema.name))}[/bold]")
    root.add(f"Package: {escape(_text(schema.package or '(none)'))}")
    ...
    for enum in _objects(schema.data.get("enums")):
        if not budget.consume(enums):
            break
        branch = enums.add(f"[bold]{escape(_text(enum.get('name', 'unnamed')))}[/bold]")
        for value in _objects(enum.get("values")):
            if not budget.consume(branch):
                break
            branch.add(
                escape(
                    f"{_text(value.get('name', 'unnamed'))} = "
                    f"{_text(value.get('number', '?'))}"
                )
            )
