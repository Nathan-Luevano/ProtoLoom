...
Each GitHub release includes a `SHA256SUMS` file covering its wheels, source
archive, and standalone binaries. Verify files downloaded into one directory
with:
...
Release distributions and binaries also carry GitHub artifact attestations.
Verify an individual file against this repository with:

```console
gh attestation verify <file> --repo Nathan-Luevano/ProtoLoom
```

Published container images include build provenance and a software bill of
materials generated by BuildKit.
